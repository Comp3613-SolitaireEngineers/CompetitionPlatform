...
def get_competitor_notifications(competitor_id):
    notifications = Notification.query.filter_by(competitor_id=competitor_id, seen=False).all()

    competitor_notifications = sorted(notifications, key=lambda x: x.timestamp, reverse=True)
    notification_info = []
